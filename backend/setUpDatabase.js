...
db.serialize(() => {
  db.run("CREATE TABLE IF NOT EXISTS accommodations (id INTEGER PRIMARY KEY, student TEXT, address TEXT, price_week INTEGER, type TEXT, background TEXT, image TEXT)");

  const stmt = db.prepare("INSERT INTO accommodations (student, address, price_week, type, background, image) VALUES (?, ?, ?, ?, ?, ?)");
